...
// (e.g. on ESC/Ctrl+C) to abort the pipeline.
...
func New(notifier *events.Notifier, runner PipelineRunner, notifications []string, workspaceDir string, plansDir string, vc VoiceConfig, ac AutonomyConfig) Model {
	// Text input
	ti := textinput.New()
...
func (m *Model) appendPlans(statusFilter string) {
	...
	m.appendBlank()
	m.appendLine(outputLine{text: fmt.Sprintf("%-10s  %-12s  %-20s  %s", "SESSION", "STATUS", "UPDATED", "NAME"), style: stylePlanHeading})
	...
	for _, p := range plans {
		...
		m.appendLine(outputLine{
			text:  fmt.Sprintf("%-10s  %-12s  %-20s  %s", p.session, p.status, p.modTime.Format("2006-01-02 15:04:05"), p.name),
			style: statusStyle,
		})
	}
	m.appendBlank()
