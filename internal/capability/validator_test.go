...
func TestCommandValidator_AllowlistMembership(t *testing.T) {
	...
	// Binary on allowlist — should pass (if it exists on PATH, otherwise LookPath fails)
	// We test with "ls" which is universally available
	result := v.Validate(types.ActionStep{
		ID:         "s1",
	...
	if !result.Valid {
		// Only fail if it's an allowlist error; LookPath errors are env-dependent
